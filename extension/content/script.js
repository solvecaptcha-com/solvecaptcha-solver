...
function doActionsOnSuccess(msg) {
    ...
    Config.getAll().then(config => {
        ...
        if (callback) {
            ...
            textarea.id = 'twcpt-callback-trigger';
...
function executeAutoSubmitRule(rules) {
    for (let i = 0; i < rules.length; i++) {
        ...
        if (regExp.test(location.href)) {
            let textarea = document.createElement('textarea');
            textarea.id = 'twcpt-autosubmit-code';
...
function createSolverButton(captchaType, widgetId, config) {
    let button = $(`
        <div class="solvecaptcha solvecaptcha_${config.buttonPosition}" data-state="ready" data-captcha-type="${captchaType}" data-widget-id="${widgetId}">
            <div class="solvecaptcha-image">
                <img src="${chrome.runtime.getURL("assets/images/icon_32.png")}">
            </div>
            <div class="solvecaptcha-info">${chrome.i18n.getMessage("solveWithSolveCaptcha")}</div>
        </div>
    `);
...
function changeSolverButtonState(button, state, message) {
    ...
    button.find(".solvecaptcha-info").text(message);
...
function getSolverButton(captchaType, widgetId) {
    return $(".solvecaptcha[data-captcha-type=" + captchaType + "][data-widget-id=" + widgetId + "]");
}
...
function showToast(message) {
    if (!$('body > .twcpt-toast-container').length) {
        $('body').append(`<div class="twcpt-toast-container"></div>`);
    }

    let toastEl = $(`
        <div class="twcpt-toast">
            <img src="${chrome.runtime.getURL("assets/images/logo.svg")}" class="twcpt-toast-logo">
            <button type="button" class="close">&times;</button>
            <div class="twcpt-toast-message">${message}</div>
        </div>
    `).appendTo('.twcpt-toast-container');
